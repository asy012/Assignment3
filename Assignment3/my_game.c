...
void fill_empty(){
  for(int r=0;r<H;r++) for(int c=0;c<W;c++) grid[r][c] = (r==0||c==0||r==H-1||c==W-1)?'#':' ';
}
...
void place_items(int lvl){
  gold_need = 3 + lvl;
  gold_have = 0;
  for(int i=0;i<gold_need;i++){
    int r,c; do{ r=1+rand()%(H-2); c=1+rand()%(W-2);} while(grid[r][c] != ' ');
    grid[r][c] = '$';
  }
  int traps = 2 + lvl/2;
  for(int i=0;i<traps;i++){ int r,c; do{ r=1+rand()%(H-2); c=1+rand()%(W-2);} while(grid[r][c] != ' '); grid[r][c]='X'; }
  en = lvl <= 1 ? 1 : (lvl <=5 ? lvl : 5 + (lvl-5)/2); if(en>MAX_EN) en=MAX_EN;
  for(int i=0;i<en;i++){ int r,c; do{ r=1+rand()%(H-2); c=1+rand()%(W-2);} while(grid[r][c] != ' '); eys[i]=r; exs[i]=c; grid[r][c]='M'; }
...
void move_enemies(){
  for(int i=0;i<en;i++){
    ...
    if(empty_cell(nr,nc) && grid[nr][nc] != 'M'){ eys[i]=nr; exs[i]=nc; }
      ...
      for(int t=0;t<4;t++){ d=rand()%4; nr=r; nc=c; if(d==0) nr--; else if(d==1) nr++; else if(d==2) nc--; else nc++; if(empty_cell(nr,nc)&&grid[nr][nc]!='M'){ eys[i]=nr; exs[i]=nc; break; } }
    }
  }
  for(int i=0;i<en;i++){ int r=eys[i], c=exs[i]; if(grid[r][c] == '@'){ } grid[r][c] = 'M'; }
}
  ...
  while(1){
    ...
    if(in){
      ...
      if(!(nx<1||ny<1||nx>W-2||ny>H-2) && grid[ny][nx] != '#'){
        ...
        if(grid[py][px] == 'X'){ printf("\033[?25h"); printf("Stepped on TRAP! GAME OVER\n"); goto END; }
        if(grid[py][px] == 'M'){ printf("\033[?25h"); printf("Caught by ENEMY! GAME OVER\n"); goto END; }
        grid[py][px] = '@';
      }
    }
    move_enemies();
    for(int i=0;i<en;i++) if(exs[i]==px && eys[i]==py){ printf("\033[?25h"); printf("Enemy hit you! GAME OVER\n"); goto END; }
    tick++; if(tick % 3 ==0) timer_moves--;
    if(timer_moves<=0){ printf("\033[?25h"); printf("Time up! GAME OVER\n"); break; }
    if(gold_have >= gold_need){ score += 50; level++; for(int r=0;r<H;r++) for(int c=0;c<W;c++) if(grid[r][c]=='M') grid[r][c]=' '; init_level(level); }
  }
END:
